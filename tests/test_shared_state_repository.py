...
def test_queued_to_failed_is_legal_audited_and_cas_protected(tmp_path: Path) -> None:
    database = _database(tmp_path)
    project_id, user_id, model_id = _seed_model(database)
    run_id = _queued_run(database, project_id, user_id, model_id)
    report = json.dumps(
        {"execution": "The private inference service could not be activated for this analysis run."},
        sort_keys=True,
    )
    ...
    assert failed["error_code"] == "INFERENCE_DISPATCH_FAILED"
    assert failed["completed_at"] is not None
    assert failed["validation_report_json"] == report
    audits = database.list_audit_events(project_id)
    assert any(
        event["action"] == "analysis.failed"
        and json.loads(str(event["details_json"])).get("error_code") == "INFERENCE_DISPATCH_FAILED"
        for event in audits
    )
