  ...
  async function refreshProjectData(projectId: string): Promise<void> {
    ...
    try {
      ...
      if (user?.is_administrator) {
        setAuditEvents(await api.listAuditEvents(projectId));
      } else {
        setAuditEvents([]);
      }
    } catch (error) {
    ...
    <section className="content-section">
      <div className="section-heading">
        <div>
          ...
          <h2>Project controls and audit trail</h2>
          <p className="muted">
            The server authorizes every administrative request. Model registration belongs to a
            project Publisher, not an administrator-only browser upload.
          </p>
        </div>
      ...
      <div className="admin-grid">
        <form className="create-project-card" onSubmit={(event) => void submit(event)}>
        ...
        <div className="audit-card">
          <div className="audit-heading">
            ...
            <span className="event-count">{auditEvents.length} events</span>
          </div>
          {auditEvents.length === 0 ? (
            <p className="muted">No audit events are recorded for this project yet.</p>
          ) : (
...
function NoProjectState({
  ...
  return (
    <section className="content-section">
      ...
      />
      <AdministrationView auditEvents={[]} onCreateProject={onCreateProject} onRefresh={() => undefined} />
    </section>
  );
}
